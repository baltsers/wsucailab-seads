 ...
 * @version $Revision: 1.12 $ $Date: 2007/09/19 02:19:52 $
 * @param <T1> is the type of the dependent entity.
 * @param <E2> is the type of the dependee entity.
 */
final class StmtAndMethodBasedDependencyXMLizer<T1, E2 extends Stmt>
	...
	@Override public void callback(final Stmt stmt, final Context context) {
		...
		try {
			...
			//System.out.println("_dependents="+_dependents+" _dependees="+_dependees);
			if (_dependents!=null && _dependees!=null && !(_dependents.isEmpty() && _dependees.isEmpty())) {
				...
				if (_dependents.size()>0 && _dependees.size()>0)  {
					//System.out.println("\n _dependents.size()="+_dependents.size()+" _dependees.size()="+_dependees.size());
					//System.out.println("_dependents="+_dependents+"\n _dependees="+_dependees);
//					for (Object dependee:_dependees)   { 
////						if (!(dependee instanceof Pair))
////							continue;
//						
//						Object _defirst = ((Pair) dependee).getFirst();
////						if (!(_defirst instanceof Stmt))
////							continue;
//						Object _deSecond = ((Pair) dependee).getSecond();
////						if (!(_deSecond instanceof SootMethod))
////							continue;
//						
//						for (Object dependent:_dependents)  {
////							if (!(dependent instanceof Pair))
////								continue;
//							
//							Object _dtfirst = ((Pair) dependent).getFirst();
////							if (!(_dtfirst instanceof Stmt))
////								continue;
//							Object _dtSecond = ((Pair) dependent).getSecond();
////							if (!(_dtSecond instanceof SootMethod))
////								continue;
//							//System.out.println("\n _dependents.size()="+_dependents.size()+" _dependees.size()="+_dependees.size());
//							final Value _de = ((DefinitionStmt) _defirst).getLeftOp();
//							final Value _dt = ((DefinitionStmt) _dtfirst).getRightOp();
//							System.out.println("Dependency edge: "+_de+", "+((SootMethod) _deSecond)+", "+((DefinitionStmt) _defirst)+" --> "+_dt+", "+((SootMethod) _dtSecond)+", "+((DefinitionStmt) _dtfirst));
