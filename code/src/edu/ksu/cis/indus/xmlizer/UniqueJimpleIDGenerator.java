 ...
 * @version $Revision: 1.21 $ $Date: 2007/02/10 19:08:38 $
 */
public final class UniqueJimpleIDGenerator
	...
	public String getIdForStmt(final Stmt stmt, final SootMethod method) {
		...
		if (method.isConcrete()) {
			@SuppressWarnings("unchecked") final Collection<Unit> _c = method.getActiveBody().getUnits();
			int _count = 0;

			for (final Iterator<Unit> _i = _c.iterator(); _i.hasNext();) {
				if (stmt == _i.next()) {
					break;
				}
				_count++;
