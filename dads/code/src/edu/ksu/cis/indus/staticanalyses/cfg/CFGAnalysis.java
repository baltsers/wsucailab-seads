...
public final class CFGAnalysis {
	...
	private Map<SootMethod, List<Unit>> method2EnclosingInvokingUnitsCache = new HashMap<SootMethod, List<Unit>>();
	/**
	...
	public boolean doesMethodLiesOnTheDataFlowPathBetween(final SootMethod method, final SootMethod srcMethod,
		...
		boolean _result = false;
		final Iterator<Unit> _invokingStmtIterator = getInvokingUnitIteratorFor(method);

		for (final Iterator<Unit> _j = _invokingStmtIterator; _j.hasNext() && !_result;) {
	...
	private Iterator<Unit> getInvokingUnitIteratorFor(final SootMethod method) {
		final Collection<Unit> _temp = method2EnclosingInvokingUnitsCache.get(method);
		final Iterator<Unit> _result;

		if (_temp == null) {
			final Iterator<Unit> _stmts = bbm.getBasicBlockGraph(method).getStmtGraph().iterator();
			final Iterator<Unit> _r = IteratorUtils.filteredIterator(_stmts,
					SootPredicatesAndTransformers.INVOKING_UNIT_PREDICATE);
			method2EnclosingInvokingUnitsCache.put(method, IteratorUtils.toList(_r));
			_result = method2EnclosingInvokingUnitsCache.get(method).iterator();
		} else {
