...
//		}
		try  {			
			if (!(stmt.toString().indexOf("Lsun/")>=0) && !(stmt.toString().indexOf("Ljava/")>=0) && !(stmt.toString().indexOf("sun/")>=0) && !(stmt.toString().indexOf("java/")>=0))
				stmt.apply(this);
		}
		catch (Exception _e)
		{
			System.out.println("Processing statement in " + stmt+" has an exception: "+_e);
		}
	}
}
