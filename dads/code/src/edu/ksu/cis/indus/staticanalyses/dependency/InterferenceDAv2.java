...
import Dads.StaticTransferGraph;
...
public class InterferenceDAv2
	...
	public InterferenceDAv2(StaticTransferGraph vtg) {
		super(vtg);
	}
	...
	@Override protected boolean isArrayDependentOn(final Pair<AssignStmt, SootMethod> dependent,
		...
		if (_result) {
			...
			if (_result)
			{
				...
				System.out.println("InterferenceDAv2() ArrayDependency edge: "+_vde+", "+_deMethod+" - "+_deStmt+" --> "+_vdt+", "+_dtMethod+" - "+_dtStmt);
	...
	@Override protected boolean isInstanceFieldDependentOn(final Pair<AssignStmt, SootMethod> dependent,
		...
		if (_result) {
			...
			if (_result)
			{
				...
				Variable _vde=vtg.makeVariable(_de,_deStmt);
				Variable _vdt=vtg.makeVariable(_dt,_dtStmt);
				System.out.println("InterferenceDAv2() InstanceDependency edge: "+_vde+", "+_deMethod+" - "+_deStmt+" --> "+_vdt+", "+_dtMethod+" - "+_dtStmt);
	...
	@Override protected boolean isStaticFieldDependentOn(final Pair<AssignStmt, SootMethod> dependent,
		...
		if (_result) {
			...
			if (_result)
			{
				...
				Variable _vde=vtg.makeVariable(_de,_deStmt);
				Variable _vdt=vtg.makeVariable(_dt,_dtStmt);
				System.out.println("InterferenceDAv2() StaticDependency edge: "+_vde+", "+_deMethod+" - "+_deStmt+" --> "+_vdt+", "+_dtMethod+" - "+_dtStmt);
