...
//		catch (Exception e) { return; }
		...
		catch (IOException e) { if (CommonReporter.outputEnabled) System.out.println(e); }
		...
		catch (Exception e) { if (CommonReporter.outputEnabled) System.out.println("DUAReporter: " + e);}
		finally {
			// IMPORTANT: clean up branch coverage array
			if (CommonReporter.cleanupPerTest)
				Arrays.fill(brCovArray, 0);
		}
	}
...
//		}
//		catch (Exception e) { Arrays.fill(duaByteArray, 0); return; }
		...
		if (hybrid) {
			if (bsBrPosCov != null) {
				...
				if (bsBrPosCov.cardinality() != 0)
					{ if (CommonReporter.outputEnabled) System.out.println("PROBLEM! some duas marked as pos-cov..."); }
				else {
					...
					for (int i = 0; i < duaIdx; ++i)
						if (bsDirCov.get(i))
							{ if (CommonReporter.outputEnabled) System.out.print(" " + i); }
					if (CommonReporter.outputEnabled)
						System.out.println();
				...
				for (int i = 0; i < duaIdx; ++i) {
					if (bsDirCov.get(i)) {
						if (bsBrPosCov.get(i))
							...
							{ if (CommonReporter.outputEnabled) System.out.println("WARNING: dir-cov dua " + i + " not cov by br!"); }
					}
					else {
						if (bsBrDefCov.get(i))
							{ if (CommonReporter.outputEnabled) System.out.println("WARNING: br-def-cov dua " + i + " not cov directly!"); }
		...
		addDummyDUAsDefsAndReportToCovMatrix(bsDirCov, duaIdx, "dua", "def", "dublock");
			...
			// IMPORTANT: clean up coverage array
			if (CommonReporter.cleanupPerTest)
				Arrays.fill(duaByteArray, (byte)0);
		}
	}
	...
	private int addDummyCov(BitSet bsCov, int duaIdx, List dummyDUAsReqBrs) {
		...
		for (Iterator itDummyDUA = dummyDUAsReqBrs.iterator(); itDummyDUA.hasNext(); ) {
			...
			for (int i = 0; i < dummyDUAReqArray.length; ++i) {
				if (brCovArrayCopy[ dummyDUAReqArray[i] ] > 0) {
					bsCov.set(duaIdx);
	...
	public static void defEvent(int varId, int defIdx, Object oContainer, int arrIdx) {
		...
		// iterate over array elements if special Integer.MIN_VALUE given for array idx (meaning new array stmt, which defines all its elements)
		if (arrIdx == Integer.MIN_VALUE) {
			final int arrLen = Array.getLength(oContainer);
			for (int i = 0; i < arrLen; ++i) {
				// store def idx for varId and (obj,idx) pair
				ObjIdxPair oiPair = new ObjIdxPair(oContainer, i);
